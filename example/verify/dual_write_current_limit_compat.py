...
def verify_single_hand(hand_info: HandInfo) -> bool:
    ...
    try:
        ...
        print(f"\n[{hand_name}] 调用 read_joint_current_limit() 读取当前值 (作为备份)...")
        original_limits = hand.read_joint_current_limit()
        print(
            f"  [{hand_name}] 当前 Current Limit (前 5 个数据):\n"
            f"  {original_limits.flatten()[:5]} ..."
        )
        ...
        print(
            f"\n[{hand_name}] 调用 write_joint_current_limit() "
            f"设置为全是 {test_limit_value} A ..."
        )
        ...
        # 3. 再次读取并验证
        print(f"\n[{hand_name}] 读取并验证设置是否生效 (交叉验证)...")

        # 使用新接口验证
        print(f"  [{hand_name}] 使用 read_joint_effort_limit() (新接口) 验证:")
        current_effort_limits = hand.read_joint_effort_limit()
        print(
            f"  [{hand_name}] 读取到的 Effort Limit: \n"
            f"  {current_effort_limits.flatten()[:5]} ..."
        )

        # 使用旧接口验证
        print(f"  [{hand_name}] 使用 read_joint_current_limit() (旧接口) 验证:")
        current_current_limits = hand.read_joint_current_limit()
        print(
            f"  [{hand_name}] 读取到的 Current Limit:\n"
            f"  {current_current_limits.flatten()[:5]} ..."
        )
...
def main(serial_numbers: Optional[list[str]] = None, auto_scan: bool = False) -> bool:
    ...
    # 打印总结
    all_pass = print_summary(results, "current_limit 兼容性验证")
    return all_pass
