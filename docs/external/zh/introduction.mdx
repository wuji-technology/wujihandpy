...
**通过环境变量配置（推荐，零代码）。** SDK 在首次构造 `Hand` 时一次性读取，之后不再生效：
...
| `WUJI_LOG_TO_CONSOLE` | `true` / `false` | `true` | 是否输出到控制台 |
...
<Callout type="warning">
  ...
  - `set_log_to_file` 仅在 logger 初始化时 `WUJI_LOG_TO_FILE` 为 `true` 才创建文件 sink。若初始化时被禁用，运行期再调用 `set_log_to_file(True)` 不会重新启用文件输出（需通过环境变量或 logger 初始化前的 `set_log_to_file(True)` 开启）。
  - `set_log_level`、`set_log_to_console` 可在任意时刻调用，立即生效。
</Callout>

#### 5.3.2 采集详细日志

默认 `info` 级别只记录关键事件。当需要分析指令收发细节（如指令下发后实际状态未跟上），将级别调至 `debug` 获取 SDO/PDO 通信记录，或调至 `trace` 额外获取原始接收字节：
