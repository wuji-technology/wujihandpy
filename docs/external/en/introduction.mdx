...
**Configure through environment variables (recommended, zero code).** The SDK reads them once at logger initialization (in the common `Hand` flow that means when the first `Hand` is constructed) and won't pick up later changes:
