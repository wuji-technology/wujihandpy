...
<Callout type="warning">
  ...
  - `set_log_to_file` opens the log file only if `WUJI_LOG_TO_FILE` is `true` at logger initialization. If file output is disabled at init, calling `set_log_to_file(True)` later won't re-enable it (enable via the environment variable, or via `set_log_to_file(True)` before logger initialization).
