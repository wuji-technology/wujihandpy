...
**Configure through environment variables (recommended, zero code).** The SDK reads them once when the first `Hand` is constructed and won't pick up later changes:
...
| `WUJI_LOG_TO_CONSOLE` | `true` / `false` | `true` | Write to console |
...
**Configure from code.** Call the `wujihandpy.logging` submodule before creating `Hand`:
...
For full API reference, see [API Reference - logging Module](../api-reference#4-logging-module).

<Callout type="warning">
  - Call `set_log_path` before the logger initializes, otherwise it raises `RuntimeError`. In the common `Hand` flow that means before creating the first `Hand`. Calling `flush()` or any other logging API also triggers logger initialization.
  - `set_log_to_file` creates the file sink only if `WUJI_LOG_TO_FILE` is `true` at logger initialization. If file output is disabled at init, calling `set_log_to_file(True)` later won't re-enable it (enable via the environment variable, or via `set_log_to_file(True)` before logger initialization).
  - `set_log_level` and `set_log_to_console` can be called at any time and take effect immediately.
</Callout>

#### 5.3.2 Collect Detailed Logs

The default `info` level captures only key events. When you need detailed command flow analysis (for example, a command was sent but the actual state didn't follow), switch to `debug` for SDO/PDO traffic, or `trace` for the raw RX bytes as well:
