...
| :--- | :--- | :--- |
| `Level.Trace` | 0 | Most verbose, includes raw RX bytes |
...
Toggle file output. The file sink is created only if `WUJI_LOG_TO_FILE` is `true` at logger initialization, and after init this function can temporarily disable or restore file output. If file output was disabled at init, calling `set_log_to_file(True)` later won't re-enable it.
...
Toggle console output. Safe to call at any time and takes effect immediately.
...
Force pending log entries to file and console. The SDK calls this automatically on process exit, so manual calls are rarely needed.
